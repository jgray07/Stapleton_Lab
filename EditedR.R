...
write.csv(routvdf, file = "test.csv")


#trying to actually run this correctly
random <-read.cross(file = "familyandrandom.csv")
...
sigj<-SNP(perm = perm, type = "mvQTL.empir.p", thresh = .005)
sigom<- SNP(perm = routv, type = "mQTL.asymp.p", thresh = .005)
...
#continuing example
matchj <- SNP.match(perm = perm, sig = sigj, type = "mvQTL.empir.p")
matchom <- SNP.match(perm = routv, sig = sigom, type = "mQTL.asymp.p")
...
#example
namesj <- SNP.name(perm = perm, match = matchj, type = "mvQTL.empir.p")
namesom <- SNP.name(perm = routv, match = matchom, type = "mQTL.empir.p")
...
write.csv(routvdf, file = "FRTest05.csv")
